...
| B | Bâtiment | $B.facteur_correctif_shab | Facteur correctif selon la surface habitable | float | Donnée calculée |
| B | Bâtiment | $B.facteur_correctif_shab_min | Facteur correctif selon la surface habitable | float | Donnée calculée |
| B | Bâtiment | $B.logements | Nombre de logements | float | 7 |
| B | Bâtiment | $B.shab | Surface habitable | float | 100 |
...
| O | Opération | $O.etas | Efficacité énergétique saisonnière (%) | float | 1.15 |
| O | Opération | $O.indice_ik | Indice de protection aux chocs (IK) | integer | 5 |
| O | Opération | $O.option_suivi_confort | Option suivi de confort | boolean | true |
| O | Opération | $O.production_solaire_utile | Production solaire utile (kWh/an) | float | 3500 |
| O | Opération | $O.puissance_nominale | Puissance nominale de l'équipement installé (kW) | float | 14 |
| O | Opération | $O.quantite | Quantité de travaux | float | 100 |
| O | Opération | $O.resistance_isolant | Resistance thermique de l'isolant installé | float | 1.2 |
| O | Opération | $O.scop | Coefficient de performance saisonnier (SCOP) | float | 4.9 |
| O | Opération | $O.superficie_capteurs_solaires | Superficie des capteurs solaires installés (m²) | float | 5 |
...
| O | Opération | $O.type_vmc_double_flux | Type de VMC double flux | AUTOREGLABLE, MODULE | TYPE_A |
| O | Opération | $O.volume_cee | Volume de CEE | float | Donnée calculée |
| O | Opération | $O.zone_climatique | Zone climatique du projet | H1, H2, H3 | H1 |
...
| O | Opération | $O.production_solaire_utile | Production solaire utile (kWh/an) | float | 5000 |
...
| O | Opération | $O.scop | Coefficient de performance saisonnier de l'équipement | float | 4.6 |
| O | Opération | $O.superficie_capteurs_solaires | Superficie des capteurs solaires installés (m²) | float | 6 |
| O | Opération | $O.taux_couverture_solaire | Taux de couverture par l’énergie solaire de l’installation (%) | float | 84 |
...
| O | Opération | $O.type_caisson_vmc_simple_flux | Type de caisson de VMC simple flux | STANDARD, BASSE_CONSOMMATION, BASSE_PRESSION | BASSE_CONSOMMATION |
...
| O | Opération | $O.consommation_suivie | Chaleur nette unile produite par l'équipement (kWh/an) | float | 25000 |
| O | Opération | $O.facteur_correctif_shab_7 | Facteur correctif selon la surface habitable | float | Donnée calculée |
| O | Opération | $O.facteur_correctif_shab_5 | Facteur correctif selon la surface habitable | float | Donnée calculée |
| O | Opération | $O.facteur_correctif_shab_4 | Facteur correctif selon la surface habitable | float | Donnée calculée |
...
| Facteur correctif | Surface habitable / Surface chauffée S en m² |
|:----------------:|:-------------------------:|
| 0.5 | S < 70 |
| 0.7 | 70 ≤ S < 90 |
| 1 | 90 ≤ S < 110 |
...
| Facteur correctif | Surface habitable S en m² |
|:----------------:|:-------------------------:|
| 0.7 | S < 70 |
| 0.9 | 70 ≤ S < 90 |
| 1 | 90 ≤ S < 130 |
